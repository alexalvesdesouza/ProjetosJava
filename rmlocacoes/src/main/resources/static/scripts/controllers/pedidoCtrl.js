...
pedido.controller("pedidoController",
	function ($scope,
		...
		$scope.pedidosEfetuadosSelecionados = [];
		$scope.pedidosEntreguesSelecionados = [];
		$scope.abaSelected = 'pedidos-efetuados';
		$scope.contrato = {
		...
		$scope.selecionaPedidoEfetuado = function (pedido) {

			var index = $scope.pedidosEfetuadosSelecionados.indexOf(pedido);
			if (index === -1) {
				$scope.pedidosEfetuadosSelecionados.push(pedido);
				return;
			}

			$scope.pedidosEfetuadosSelecionados.splice(index, 1);
		};

		$scope.selecionaPedidoEntregue = function (pedido) {

			var index = $scope.pedidosEntreguesSelecionados.indexOf(pedido);
			if (index === -1) {
				$scope.pedidosEntreguesSelecionados.push(pedido);
				return;
			}

			$scope.pedidosEntreguesSelecionados.splice(index, 1);
		};

		var deparaPedidos = function(listaPedidos) {
			var pedidos = [];
			angular.forEach(listaPedidos, function (value, key) {
		...
		$scope.efetuarDevolucaoPedidosSelecionados = function () {
			
			var pedidosADevolver = $scope.pedidosEntreguesSelecionados;
			var pedidos = deparaPedidos(pedidosADevolver);
			$http.put("/pedidos/registrar-devolucao-massa", pedidos)
			.success(function (pedidoFinalizado) {
				carregarPedidos();
				limparForm('pedidoForm');
				Materialize.toast('Pedidos devolvidos com sucesso.', 4000, 'rounded');
		...
		$scope.efetuarEntregaPedidosSelecionados = function () {
			
			var pedidosADevolver = $scope.pedidosEfetuadosSelecionados;
			var pedidos = deparaPedidos(pedidosADevolver);
			$http.put("/pedidos/registrar-entrega-massa", pedidos)
				.success(function (pedidoFinalizado) {
					carregarPedidos();
					limparForm('pedidoForm');
					Materialize.toast('Pedidos entregues com sucesso.', 4000, 'rounded');
		...
		var validaCamposObrigatorios = function() {
			var pedido = $scope.pedido;
			if (pedido.dtaEntrega === '' || pedido.dtaEntrega === undefined) {				
				Materialize.toast('Campo Data de Entrega é obrigatório', 4000, 'rounded');
			}
			if (pedido.dtaDevolucao === '' || pedido.dtaDevolucao === undefined) {				
				Materialize.toast('Campo Data de Devolução é obrigatório', 4000, 'rounded');
			}
		};
		...
		$scope.abaSelecionada = function (abaSelecionada) {
			...
			if ('pedidos-efetuados' === abaSelecionada) {
				carregarPedidos();
				return;
			}
			if ('pedidos-efetuados' === abaSelecionada) {
		...
		var carregarPedidos = function () {
			var pedidoStatus = $scope.abaSelected;
			$http.get("/carregarPedidos/" + pedidoStatus)
				.success(function (ret) {
					$scope.pedidos = [];
					$scope.pedidos = ret;
					numberOfPages();
